...
bc_count_file <- "../01_counting-barcodes/AAV-lib1/output/merged-barcode-counts.tsv.gz"
bc_info_file <- "Lib1_Overview_Sascha.csv"
sample_info_file <- "Lib1_cell_counts.csv"

#' Read all files:
sample_info <- read_csv( sample_info_file, col_types = "cccil") %>%
  mutate( sample_n = paste0("Sample", sample_n))
...
bc_info <- read_csv( bc_info_file,
                     col_names = c("AAV_ID", "barcode_seq", "AAV_lib_read_percent"),
                     col_types = "ccd", skip = 1) %>%
  mutate( AAV_lib_scalefactor = mean(AAV_lib_read_percent) / AAV_lib_read_percent,
          AAV_ID = stringr::str_replace( AAV_ID, "WHc", "AAV"))
...
bc_props %>% filter( AAV_ID %in% topX) %>%
  ...
  ylim( 0, .265) +
  geom_point( data = bc_props %>% filter( AAV_ID %in% topX, norm_proportion > .28),
              mapping = aes( x = AAV_ID), y = .28, shape = 17, size = 1) +
  theme( panel.grid.major.x = element_blank(), panel.grid.minor = element_blank())
ggsave("plots/AAVlib1_overall-top10-boxplots.png", width=5, height=3, dpi=400)
ggsave("plots/AAVlib1_overall-top10-boxplots.svg", width=5, height=3, dpi=400)
...
bc_props %>% filter( AAV_ID %in% topX) %>%
  ...
  geom_point( data = bc_props %>% filter( AAV_ID %in% topX, norm_proportion > .28),
              mapping = aes( x = AAV_ID), y = .28, shape = 17, size = 1) +
  theme( panel.grid.major.x = element_blank(), panel.grid.minor = element_blank())
ggsave("plots/AAVlib1_overall-top10-barplots-median.png", width=5, height=3, dpi=400)
ggsave("plots/AAVlib1_overall-top10-barplots-median.svg", width=5, height=3, dpi=400)
...
bc_props %>% filter( AAV_ID %in% topX_mean) %>%
  ...
  geom_point( data = bc_props %>% filter( AAV_ID %in% topX_mean, norm_proportion > .28),
              mapping = aes( x = AAV_ID), y = .3, shape = 17, size = 1) +
  theme( panel.grid.major.x = element_blank(), panel.grid.minor = element_blank())
ggsave("plots/AAVlib1_overall-top10-barplots-mean.png", width=5, height=3, dpi=400)
ggsave("plots/AAVlib1_overall-top10-barplots-mean.svg", width=5, height=3, dpi=400)
...
bc_props_celltype %>%
  ...
  geom_point( data = bc_props %>% filter( AAV_ID %in% topX_mean, norm_proportion > .38),
              y = .4,  shape = 17, size = 1.5, color = "black") +
  scale_x_reordered() +
  ...
  labs( y = "normalized proportion", x = NULL) + guides( color=F, fill=F)
ggsave("plots/AAVlib1_top10-perCelltype-barplots-mean.png", width=10, height=6, dpi=400)
ggsave("plots/AAVlib1_top10-perCelltype-barplots-mean.svg", width=10, height=6, dpi=400)
...
bc_props_invitro %>%
  ...
  scale_fill_manual( values = aav_colors) +
  theme( panel.grid.major.x = element_blank(), panel.grid.minor = element_blank())
ggsave("plots/AAVlib1_inVitro-top10-barplots.png", width=5, height=3, dpi=400)
ggsave("plots/AAVlib1_inVitro-top10-barplots.svg", width=5, height=3, dpi=400)



#' Export all our info as CSV files:
bc_props %>% bind_rows( bc_props_invitro) %>%
  dplyr::rename(`AAV%_of_transduction_library`=AAV_lib_read_percent, AAV_normalisation_factor=AAV_lib_scalefactor,
  ...
  write_csv("tables/AAVlib1_AAV-barcode-counts.csv")
...
bc_props %>%
  ...
  write_csv("tables/AAVlib1_AAV-barcode-proportions-by-celltype.csv")
...
bc_props %>%
  ...
  summarize( n = n(),
             ...
             IQR_upper = quantile( norm_proportion, 3/4)) %>% ungroup %>%
  arrange( -mean) %>%
  write_csv("tables/AAVlib1_AAV-barcode-proportions_InVivo.csv")
...
bc_props %>%
  ...
  dplyr::select( serotype=AAV_ID, average, starts_with("aN"), starts_with("qN"), starts_with("Pro"),
                 starts_with("Neur"), starts_with("Ast"), starts_with("Oli"), starts_with("Epe")) %>%
  write_tsv( "tables/supplement/AAVlib1_overall_serotype_proportions.tsv")
